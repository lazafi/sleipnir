...
public class DLSResearch_EdgeOnly extends OffloadScheduler {

	public DLSResearch_EdgeOnly(MobileApplication A, MobileCloudInfrastructure I) {
		super();
	...
	public DLSResearch_EdgeOnly(Tuple2<MobileApplication,MobileCloudInfrastructure> t) {
		super();
		...
		while(tasks.size() > 0) {
			for (MobileSoftwareComponent t: taskPool) {
				if(!t.isOffloadable())  {
				   ...
				   for(EdgeNode cn : currentInfrastructure.getEdgeNodes().values()) {
